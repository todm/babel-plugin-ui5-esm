/*
Original code from @babel/plugin-transform-modules-amd

[Changes]
- Canged wrapper to use sap.ui.define instead of define
- Changed behaviour when exporting a singular default export.
    the value will directly be set to module.exports instead of exports.default
- Checks if sap.ui.define already exists
*/
...
const { getImportSource } = require('babel-plugin-dynamic-import-node/utils');
...
function getSingleDefaultExport(meta) {
    if (!hasExports(meta)) return;
    let defaultExport = Array.from(meta.local).find(([, val]) => val.names.includes('default'));
    if (defaultExport) {
        defaultExport = {
            var: defaultExport[0],
            ...defaultExport[1]
    ...
    return !!defaultExport && meta.local.size === 1 ? defaultExport : false;
}

function isUi5ModuleAlready(path) {
    if (!path.scope.globals['sap']) return false;
    let hasSapDefine = false;
    path.traverse({
        CallExpression(cePath) {
            if (!hasSapDefine && cePath.getSource().startsWith('sap.ui.define')) {
                hasSapDefine = true;
            }
        }
    });
...
module.exports = declare((api, options) => {
    ...
    return {
        ...
        visitor: {
            ...
            Program: {
                exit(path, { requireId }) {
                    if(isUi5ModuleAlready(path)) return;

                    if (!isModule(path)) {
                        if (requireId) {
                            injectWrapper(path, buildAnonymousWrapper({ REQUIRE: t.cloneNode(requireId) }));
                        }
                        return;
                    }
                    ...
                    if (hasExports(meta)) {
                        const singleDefaultExport = getSingleDefaultExport(meta);
                        if (singleDefaultExport) {
                            const moduleIdentifier = path.scope.generateUidIdentifier('module');
                            amdArgs.push(t.stringLiteral('module'));
                            importNames.push(moduleIdentifier);
                            path.node.body.push(
                                template('MODULE.exports = DEFAULTEXPORT')({
                                    MODULE: moduleIdentifier,
                                    DEFAULTEXPORT: t.identifier(singleDefaultExport.var)
                                })
                            );
                        }
                        ...
                        if (!isSideEffectImport(metadata)) {
                            ...
                            if (interop) {
                                const header = t.expressionStatement(
                                    t.assignmentExpression('=', t.identifier(metadata.name), interop)
                                );
